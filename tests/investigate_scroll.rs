...
fn investigate_scroll_on_different_panels() {
    ...
    println!(
        "Initial - Panel: {:?}, selected_log: {}",
        app.current_panel, app.selected_log
    );
    ...
    for i in 0..3 {
        app.handle_key(KeyEvent::new(KeyCode::Char('j'), KeyModifiers::NONE))
            .ok();
        println!(
            "After 'j' #{} on Status - selected_log: {}",
            i + 1,
            app.selected_log
        );
    }
    ...
    println!(
        "\nFinal selected_log: {} (max index should be {})",
        app.selected_log,
        app.logs.len() - 1
    );
    ...
    assert!(
        ...
        "BUG: selected_log went out of bounds!"
    );
    ...
    println!(
        "\nFinal selected_log: {} (min should be 0)",
        app.selected_log
    );
}
...
fn investigate_scroll_on_empty_logs() {
    ...
    if app.logs.is_empty() && app.selected_log > 0 {
        panic!(
            "BUG: selected_log is {} but logs are empty!",
            app.selected_log
        );
    }
}
